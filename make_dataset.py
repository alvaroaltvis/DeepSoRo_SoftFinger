...
## This file needs to be used in two ways. When first importing data it is important ot create the file DATASET_# manually. Then it is important to uncomment to see the length of the pointcloud. And after that we can crop them and send them to the DATASET 
## Also change the ranges to upload every image
...
class MAKE_DATASET():
    ...
    def __init__(self):
        ...
        # create output path 
        for i in range(300): # Adjust number to number of samples 
            self.output_path = os.path.join(OUTPUT_PREFIX, f'_{i}.npz')
            np.savez(self.output_path, img=image_seq[i], pcd=pcd_seq[i])
        
    def load_files(self):
        imgs_path = [os.path.join(self.image_path, f'{i}.jpg') for i in range(0, 300)] # Adjust number to number of samples 
        pcds_path = [os.path.join(self.pcd_path, f'data_{i}.npz') for i in range(0, 300)] # Adjust number to number of samples
        
        # container to store data
        img_seq = np.zeros((self.simulation_length, 1, 256, 256))
        pcd_seq = np.zeros((self.simulation_length, 4510, 3))  # Change depending on the size of the pointcloud 
    ...
    def load_pcd_from_file(self, filename):
        ...
        # Check min size of pointcloud Downsize pointcloud
        # print(len(vertices)) #-- See what the min points in a pointcloud is. Do this before eliminating the extra points
        # lenght.append(len(vertices)) # Uncomment print(min(lent)) in the name function to see the minimu value 

        # Eliminate the extra point in each pointcloud
        rng = default_rng()
        extra_points=len(vertices) - 4510 # The number needs to be changed to what the len(vertices) gives on the commands above
